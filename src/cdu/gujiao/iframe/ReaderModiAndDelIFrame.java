...
public class ReaderModiAndDelIFrame extends JInternalFrame {
	...
	public ReaderModiAndDelIFrame() {
		...
		final DefaultTableModel model=new DefaultTableModel();	//创建表格视图模型	
		Object[][] results=getFileStates(Dao.selectReader());	//获得表格将要显示的数据的数组
		model.setDataVector(results,columnNames);				//添加表格视图模型的数据
		
		table = new JTable();
		table.setModel(model);									//设置表格的视图模型
		scrollPane.setViewportView(table);						//将表格添加到scrollpane
		table.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);		//设置表格可以自动增大
		table.addMouseListener(new TableListener());			//添加表格的监听事件
